...
6. **Variables de Plantilla:** Soporta `{not_type}` (canal: "notification", "sms", "imap"), `{not_title}` / `{sms_sender}` (remitente), `{not_text}` / `{sms_text}` (contenido del mensaje SMS o notificación), `{package_name}`, `{timestamp}`, `{device_uuid}`, `{imap_subject}`, `{imap_body}` y `{global_NOMBRE}`.
7. **Importación y Exportación JSON (Versionado):** Administrado por `RuleJsonUtil.kt`. El formato exportado es un objeto raíz con la estructura `{"version": 1, "rules": [...]}`. Las cabeceras HTTP se exportan en un array de objetos `"headers": [{"header": "Key", "value": "Val"}]` y las variables globales usadas se detectan e inyectan automáticamente en el array `"_vars": [{"name": "VAR", "value": "VAL"}]`. Al importar, se valida que la versión sea mayor o igual al mínimo soportado (versión 1).
8. **Filtro Regex por Campos (regexMatchFields):** Permite configurar los campos específicos contra los que se evalúa la expresión regular de cada regla:
   - **Aplicaciones (APP):** `title` (Título), `text` (Texto) o ambos.
   - **SMS:** `sender` (Remitente), `recipient` (Destinatario) o `body` (Contenido).
   - **IMAP (Gmail):** `from` (Emisor), `to` (Receptor), `subject` (Asunto) o `body` (Cuerpo).
9. **Integración IMAP (Email):** Intercepta notificaciones de la app de Gmail (`com.google.android.gm`) para activar `ImapFetchWorker`, que descarga los correos no leídos (`UNSEEN`) utilizando JavaMail y los procesa asegurando idempotencia con registro local.
