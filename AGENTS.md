...
6. **Variables de Plantilla:** Soporta `{not_title}` / `{sms_sender}` (remitente), `{not_text}` / `{sms_text}` (contenido del mensaje SMS o notificación), `{package_name}`, `{timestamp}`, `{imap_subject}`, `{imap_body}` y `{global_NOMBRE}`.
